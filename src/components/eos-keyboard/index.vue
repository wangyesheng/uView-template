  ...
  <view class="xm-keyboard">
    <view class="xm-content" v-if="show">
      ...
      <view class="xm-box" :class="anim ? 'xm-animation' : ''">
          ...
          <view class="xm-toolbar-left">
            <view class="keyboard-input" @tap.stop="changeCur()">
              <text
                ...
                :key="index"
                :class="cur === index ? 'cur abs' : ''"
                @tap.stop="changeCur(index)"
                >{{ item }}</text
              >
              <text v-if="input.length === 0" class="cur"></text>
            </view>
            <view v-if="input.length" class="xm-tips"
              >({{ input.length }}位)</view
            >
          </view>
        ...
        <view
          class="xm-main"
          :class="['xm-mode-' + typeInfo.mode]"
          :style="{ paddingBottom: safeBottom + 'px' }"
        >
          <view class="xm-main-left">
            <view
              class="xm-line"
              v-for="(lines, lineIndex) in list"
              :key="lineIndex"
            >
              <view
                class="xm-btn"
                ...
                :key="index"
                :class="disable.indexOf(item) !== -1 ? 'disable' : ''"
                @tap="keyPress(item)"
                :style="{
              ...
              >
                ...
                <view class="high-item" v-if="highItem == item">{{
                  item
                }}</view>
              </view>
            </view>
            ...
            <view class="xm-line">
              <view
                ...
                :style="{
                  ...
                  height:
                    typeInfo.mode == 1 ? keyWidth * proportion + 'px' : '100%',
                  maxHeight: typeInfo.mode == 1 ? '' : 'unset',
                  marginTop:
                    typeInfo.mode == 1 ? '' : (index != 0 ? 0 : 20) + 'upx',
                }"
              >
...
export default {
  ...
  computed: {
    ...
    minWidth() {
      return (
        this.width /
        ((this.max + this.typeInfo.mode - 1) * this.ratio + this.max + 1)
      );
    },
    keyWidth() {
  ...
  mounted() {
    const { windowWidth, windowHeight, safeAreaInsets } =
